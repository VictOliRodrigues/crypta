...
describe('security-headers.conf', () => {
   ...
   *
   * Duas sutilezas, ambas descobertas por este teste reprovando:
   *
   * 1. a asserção precisa olhar só o valor da diretiva, não o arquivo inteiro —
   *    senão um comentário que *menciona* `'unsafe-eval'` derruba o teste;
   * 2. o padrão exige a aspa imediatamente antes de `unsafe-eval`, então
   *    `'wasm-unsafe-eval'` — que o ADR 0016 autoriza — não dispara.
   */
  it('nunca permite unsafe-eval puro na CSP', () => {
    const policy = /add_header Content-Security-Policy "([^"]*)"/.exec(snippet)?.[1];
