...
**`TABLES` esquecida** não quebra nada de imediato: a suíte continua verde e passa a vazar estado entre specs, e a falha aparece depois, em um teste que não mudou. `harness-guard.e2e-spec.ts` compara a lista com `information_schema` e falha nomeando a diferença nos dois sentidos, então esta linha da checklist deixou de depender de alguém lembrar dela.
